 ...
 *
 * Adaptação VOS (15/09/2026), ÚNICA mudança em relação ao original: os dois
 * `inert` do JSX viraram string vazia. Este projeto está no React 18, que não
 * conhece o atributo booleano `inert` . com `inert` puro ele avisa no console e
 * NÃO escreve o atributo. `inert=""` é o mesmo atributo em HTML e passa direto.
 * Ao subir pro React 19, dá pra voltar ao `inert` cru.
 */
...
export default function GlyphPortal({
  ...
  useLayoutEffect(() => {
    ...
    /* Onde a palavra senta na altura do palco. Era 0.46 cravado; no CELULAR ela
       sobe pra 0.38 (founder 15/09: "coloca o modo zé mais pra cima também no
       mobile, tá muito lá embaixo") . em tela alta e estreita, 46% com a frase
       por cima deixava meia tela de preto antes de qualquer coisa acontecer. */
    let ancoraY = 0.46;
    ...
    const paint = (progress: number) => {
      ...
      const transform = `translate(${W / 2} ${H * ancoraY + H * 0.04 * eased}) scale(${scale}) rotate(${roll}) translate(${-cx} ${-cy})`;
      ...
      const dx = W / 2 / scale, dy = (H * ancoraY + H * .04 * eased) / scale;
    ...
    const layout = () => {
      ...
      /* Tela estreita = palavra mais alta: é o mesmo palco, mas sem meia tela de
         preto antes dela. */
      ancoraY = W < 760 ? 0.38 : 0.46;
      H = motion.matches ? Math.min(viewportHeight * 0.75, 480) : viewportHeight;
      section.style.setProperty("--gp-height", `${H}px`);
      /* CURSO MAIS CURTO NO CELULAR (founder 15/09: "encurta o palco no
         celular"): o mergulho inteiro cabe em 62% de uma tela de rolagem, em
         vez de uma tela cheia . a câmera entra com menos dedo e a seção come
         menos página.

         É o CURSO que encolhe, não o palco: encurtar a altura do quadro (o
         caminho óbvio) tira o preto de baixo mas descola o cenário da tela .
         medido em 390, sobrava uma faixa preta de 152px embaixo no meio do
         mergulho, porque o palco preso deixa de cobrir a janela. */
      travel = H * length * (W < 760 ? 0.62 : 1);
      ...
      for (const button of buttons) {
        ...
        Object.assign(button.style, {
          ...
          top: `${H * ancoraY + (letter.y - center.y) * startScale - Math.max(0, 44 - letter.height * startScale) / 2}px`,
          width: `${Math.max(1, letter.width * startScale)}px`,
          height: `${Math.max(44, letter.height * startScale)}px`,
        });
      }
      section.style.setProperty("--gp-word-top", `${H * ancoraY - bounds.height * startScale / 2}px`);
      section.style.setProperty("--gp-word-bottom", `${H * ancoraY + bounds.height * startScale / 2}px`);
